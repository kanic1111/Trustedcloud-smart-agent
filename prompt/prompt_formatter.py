class PromptFormatter:
    ...
    def k8s_instruction_prompt_ch(self, context, query):
        ...
        messages = [
            {"role": "system", "content": system_prompt},
            {"role": "assistant", "content": f"參考資料：\n{context}"}
        ]

        return messages

    def k8s_instruction_prompt(self, context, query , user_input=""):
        """
        專為多段混合操作教學內容設計，並強化圖片路徑保留與段落對應機制。
        可以加在開頭
...
Select and present **only the steps** from the RAG reference materials that are **most relevant to the user’s question**.
Do **not** create, modify, or invent any content that does not already exist in the RAG source.

---

### 📘 Input Description

* The RAG system provides several tutorial articles, each containing step-by-step instructions and images marked as `(Image：./...)`.
* The user’s query describes what they want to accomplish.

---

### 🧩 Task Rules

1. Identify the **most relevant steps** from the RAG materials that best answer the user’s question.
2. **Use only the original content** from the references and it must be (text + image tags).
3. **Do not** modify, translate, or invent new images — keep `(Image：...)` unchanged.
4. Preserve the **original sequence** of selected steps from their source. Do not reorder within an article.
...
* Use numbered steps:
  `1. <step text> (Image：./filename.png)`
  `2. <step text>`
* Each step ends with its corresponding `(Image：...)` if available.
* Do **not** include any headings, introduction, or user query in the output.
* Output should be markdown format.
* Optional notes or warnings can be included **only if** they appear in the reference.
* Add  "You Can see more instruction in [Trusted-Cloud User Guide](https://docs.trusted-cloud.nchc.org.tw/s/user-guide-en)" at the very end.
---

### ⚠️ Critical Constraints

* Never invent, summarize, or merge steps.
* Never reorder content across sources.
* Only output what already exists in the RAG materials.
* Choose the **most contextually relevant** part based on the user’s question.
            '''
        )

        messages = [
            {"role": "system", "content": system_prompt},
            {"role": "user", "content": "Load RAG context."}, 
            {"role": "assistant", "content": f"參考資料：\n{context}"}
        ]
    ...
    def query_rewriting_prompt_format(self, context, query):
        ...
        system_prompt = (
            ...
            "   - Networking & security group (firewalls, security groups, floating IP, load balancing)\n"
    ...
    def vm_instruction_prompt(self, context: str, options: dict) -> str:
        ...
        You are a cloud VM assistant, responsible for guiding the user step by step to configure the five required VM parameters, and for calling the corresponding tool function after each parameter is confirmed to record it.
            ...
            - the current status shouldn't be display if you didn't use any Toolcalls.
            - If user ask you to give them a recommand setup feel free to ask them the spec.   
            ...
            - 🚫 Do not decide or change parameters on your own without user's permission.  
            ...
            - 🚫 Do not Answer question other than creating VM. 
            ...
            - ✅ If User tell you to recommand setup fill parameters  and wait for user confirmation, if user question too simple ask them to give more detail
    ...
    def jupyter_instruction_prompt(self, context: str, options: dict) -> str:
        ...
        You are a cloud Jupyter assistant, responsible for guiding the user step by step to configure the seven required Jupyter parameters, and for calling the corresponding tool function after each parameter is confirmed to record it.
            ...
            Prohibited Actions:  
            - 🚫 Do not fill in any parameter values proactively.  
            - 🚫 Do not decide or change parameters on your unless the user answered confirm chosen parameters.  
            - 🚫 Do not create Jupyter before explicit confirmation from the user.  
            - 🚫 Do not include any tool or function names in replies. Tool calls can only be executed at the system level.  
            - 🚫 Do not Answer question other than creating Jupyter or applications, if detect tell them you are a Jupyter assistant and can help user create application. 
            - 🚫 Do not show the parameter without the toolcalling.
            Notes:  
            ...
            - ✅ Replies must primarily be in English. If the user inputs in a non-English language, translate it into English before responding.  
            - ✅ If User tell you to recommand setup for deploy, you can ask if user want you to fill the parameters and wait for user confirmation before apply, if user question too simple ask them to give more detail
    ...
    def format_prompt(self, context, query, style="basic", user_input=""):
        """
        根據 style 參數選擇不同格式
        """
